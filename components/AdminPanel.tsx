...
const AdminPanel: React.FC<AdminPanelProps> = ({ projects, onLogout, onBack, onAdd, onDelete }) => {
  ...
  const generateConstantsCode = () => {
    ...
    copyToClipboard(code, 'Constants Code Copied!');
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, type: 'image' | 'video') => {
    ...
    if (file.size > maxSize * 1024 * 1024) {
      alert(`File exceeds current threshold (${maxSize}MB).`);
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!newProject.title || !newProject.imageUrl) {
      alert("A Title and Cover Image are mandatory.");
      return;
    }
    ...
    <div className="min-h-screen bg-[#050505] p-4 md:p-12 text-zinc-300 font-['Space Grotesk']">
            ...
            <div className="space-y-6 sticky top-12">
              <div className="glass p-8 rounded-[40px] border border-white/5 bg-zinc-950/40 shadow-2xl">
                <div className="flex items-center justify-between mb-8">
                  <h2 className="syncopate text-[10px] font-bold tracking-[0.3em] text-white uppercase">Global Sync Hub</h2>
                  <div className="flex h-2 w-2 rounded-full bg-green-500 shadow-[0_0_10px_#22c55e]"></div>
                </div>
                
                <p className="text-[10px] text-zinc-500 mb-6 leading-relaxed">
                  To make your changes visible to **everyone**, copy the code below and paste it into <code className="text-indigo-400">components/constants.tsx</code>.
                </p>

                <div className="flex flex-col gap-3">
                  <button 
                    onClick={generateConstantsCode}
                    className="w-full py-4 glass border border-indigo-500/30 text-white font-bold rounded-2xl text-[9px] uppercase tracking-[0.2em] hover:bg-indigo-500/20 transition-all flex items-center justify-center gap-3"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path></svg>
                    Copy for constants.tsx
                  </button>
                  {copyStatus && (
                    <div className="text-center animate-bounce">
                      <span className="text-[9px] font-bold text-green-400 uppercase tracking-widest">{copyStatus}</span>
                    </div>
                  )}
                </div>
              </div>

              <div className="glass p-8 rounded-[40px] border border-white/5 bg-zinc-950/40 max-h-[60vh] overflow-y-auto custom-scrollbar shadow-2xl">
                <div className="flex items-center justify-between mb-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <div className="space-y-6">
                    <div className="space-y-4">
                      ...
                      <div className="group">
                        <label className="text-[9px] font-black uppercase tracking-[0.3em] text-zinc-500 mb-2 block group-focus-within:text-indigo-400 transition-colors">Narrative</label>
                        <textarea 
                    ...
                    <div className="p-6 rounded-[28px] border border-white/5 bg-zinc-900/20 space-y-5">
                      <div>
                        <label className="text-[8px] font-bold uppercase tracking-widest text-zinc-600 mb-1.5 block">Image CDN/Data</label>
                        <div className="relative">
                          <input 
                            type="text" 
                            value={newProject.imageUrl} 
                            onChange={e => setNewProject({...newProject, imageUrl: e.target.value})} 
                            className="w-full bg-black/40 border border-white/5 rounded-xl px-5 py-3 outline-none focus:border-indigo-500/30 text-[11px] text-zinc-400 font-mono" 
                          />
                          <button type="button" onClick={() => fileInputRef.current?.click()} className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-700 hover:text-white transition-colors">
                      ...
                      <div>
                        <label className="text-[8px] font-bold uppercase tracking-widest text-zinc-600 mb-1.5 block">Video Resource (Optional)</label>
                        <div className="relative">
                          <input 
                            type="text" 
                            ...
                            className="w-full bg-black/40 border border-white/5 rounded-xl px-5 py-3 outline-none focus:border-indigo-500/30 text-[11px] text-zinc-400 font-mono" 
                          />
                          <button type="button" onClick={() => videoInputRef.current?.click()} className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-700 hover:text-white transition-colors">
                  ...
                  <button 
                    ...
                    className="w-full py-5 bg-white text-black font-black rounded-2xl transition-all uppercase tracking-[0.4em] text-[10px] cursor-pointer shadow-[0_20px_40px_rgba(255,255,255,0.1)] hover:bg-zinc-200 disabled:opacity-50"
                  >
                    {isUploading === 'saving' ? 'Processing...' : 'Add to Local Drafts'}
                  </button>
                </form>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {projects.length === 0 ? (
                <div className="col-span-full py-32 text-center glass rounded-[40px] border-dashed border-2 border-zinc-900 flex flex-col items-center justify-center gap-6">
                  <span className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-800">No projects currently tracked.</span>
                </div>
                ...
                <div key={project.id} className="glass p-5 rounded-[32px] flex gap-5 items-center border border-white/5 relative overflow-hidden group hover:border-indigo-500/30 transition-all hover:-translate-y-1">
                  {deletingId === project.id && (
                    <div className="absolute inset-0 bg-[#050505]/95 backdrop-blur-md flex flex-col items-center justify-center z-20 text-red-500 text-[8px] font-black uppercase tracking-widest">
                      Purging...
                    </div>
                  )}

                  <div className="w-20 h-20 rounded-2xl overflow-hidden bg-zinc-900 border border-white/5 flex-shrink-0">
      ...
      {projectToPurge && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="relative glass p-10 rounded-[48px] border border-red-500/20 max-w-md w-full text-center animate-in zoom-in-95 duration-300">
            <h3 className="syncopate text-xl font-bold text-white mb-4 uppercase">Initiate Purge?</h3>
            <p className="text-zinc-500 text-sm mb-10 font-light">Confirm deletion of <b>"{projectToPurge.title}"</b> from local storage.</p>
            <div className="flex flex-col gap-3">
              <button onClick={executePurge} className="w-full py-5 bg-red-600 text-white font-black rounded-2xl uppercase tracking-[0.3em] text-[10px] cursor-pointer">Purge Record</button>
              <button onClick={() => setProjectToPurge(null)} className="w-full py-5 glass text-zinc-400 font-black rounded-2xl uppercase tracking-[0.3em] text-[10px] cursor-pointer">Abort</button>
            </div>
          </div>
        </div>
      )}
